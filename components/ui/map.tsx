...
const Map = forwardRef<MapRef, MapProps>(function Map({ children, className, theme: themeProp, styles, projection, viewport, onViewportChange, loading = false, ...props }, ref) {
...
function DefaultMarkerIcon() {
  return <div className="relative h-4 w-4 rounded-full border-2 border-primary bg-[#a992cf] shadow-lg" />;
}
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  return createPortal(<div className={cn("bg-black/60 backdrop-blur-md border border-white/10 rounded-xl px-4 py-3 flex flex-col gap-2 text-xs text-white", className)}>{children}</div>, container);
}
